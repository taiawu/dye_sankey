...
# tribble(
#   ~.filter_col,      ~.corr_col,    ~filt_var,   ~rep_var,  ~overwrite,
#      node_group,  y_correction,      "start",     500,       FALSE
# )
nodes  %>% ungroup() %>% select(node_name) %>% table()
nodes%>% ungroup()  %>% select(node_group) %>% table()

nodes %>% 
  ungroup() %>%
  arrange(node_name) %>%
  mutate(node_side = rep(c("node_start", "node_end"), times = nrow(.)/2)) %>%
  filter(node_side == "node_end" & node_group == "library")
  mutate(xvalue = replace(xvalue, xvalue > 8, xvalue[xvalue > 8] + 5))

nodes %>% 
  ungroup() %>%
  arrange(dye, node_name, xvalue)
...
nodes %>% 
  ...
  filter(! node_group %in% c("start", "library"))
```
...
nodes %>% 
  ...
  unite("node_group_side", c("node_group", "node_side"), sep = "_", remove = FALSE ) %>%
  unite("node_name_side", c("node_name", "node_side"), sep = "_", remove = FALSE ) %>%
   # mutate(xvalue_final = replace(xvalue_final, 
   #                               node_side == "node_end" & node_group == "library", 
   #                               xvalue_final[node_side == "node_end" & node_group == "library"] + 1)) %>%
   # 
   #   mutate(xvalue_final = replace(xvalue_final, 
   #                               node_side == "node_start" & node_group == "start", 
   #                               xvalue_final[node_side =="node_start" & node_group == "start"]  - 0.3)) %>%
  # arrange(xvalue) %>%
 
  
 # nudge_node( . , .filter_col = node_group, .corr_col = yvalue_final, filt_var = "add_T", rep_var = 400, overwrite  = FALSE) %>%
...
nudge_node( . , .filter_col = node_name, .corr_col = yvalue_final, filt_var = "add_T", rep_var = 360, overwrite  = FALSE)  %>%
nudge_node( . , .filter_col = node_name, .corr_col = xvalue_final, filt_var = "add_T", rep_var = 1.5, overwrite  = FALSE)  %>%
  
nudge_node( . , .filter_col = node_name, .corr_col = yvalue_final, filt_var = "add_C", rep_var = 320, overwrite  = FALSE)  %>%
nudge_node( . , .filter_col = node_name, .corr_col = xvalue_final, filt_var = "add_C", rep_var = -0.5, overwrite  = FALSE)  %>%
  
nudge_node( . , .filter_col = node_name, .corr_col = yvalue_final, filt_var = "add_A", rep_var = 330, overwrite  = FALSE)  %>%
nudge_node( . , .filter_col = node_name, .corr_col = xvalue_final, filt_var = "add_A", rep_var = -0.7, overwrite  = FALSE)  %>%

nudge_node( . , .filter_col = node_name, .corr_col = yvalue_final, filt_var = "add_L", rep_var = 380, overwrite  = FALSE)  %>%
nudge_node( . , .filter_col = node_name, .corr_col = xvalue_final, filt_var = "add_L", rep_var = -2.5, overwrite  = FALSE)  %>%
  

nudge_node( . , .filter_col = node_name, .corr_col = yvalue_final, filt_var = "add_MWA", rep_var = +5, overwrite  = FALSE)  %>%
nudge_node( . , .filter_col = node_name, .corr_col = xvalue_final, filt_var = "add_MWA", rep_var = -1.5, overwrite  = FALSE)  %>%
  
nudge_node( . , .filter_col = node_name, .corr_col = yvalue_final, filt_var = "add_SYPROlog", rep_var = -30, overwrite  = FALSE)  %>%
nudge_node( . , .filter_col = node_name, .corr_col = xvalue_final, filt_var = "add_SYPROlog", rep_var = 0.5, overwrite  = FALSE)  %>%
  
nudge_node( . , .filter_col = node_name, .corr_col = yvalue_final, filt_var = "add_MWC", rep_var = -60, overwrite  = FALSE)  %>%
nudge_node( . , .filter_col = node_name, .corr_col = xvalue_final, filt_var = "add_MWC", rep_var = -3, overwrite  = FALSE)  %>%
   
nudge_node( . , .filter_col = node_group, .corr_col = yvalue_final, filt_var = "library", rep_var = 100, overwrite  = FALSE)  %>% 
  
nudge_node( . , .filter_col = node_group, .corr_col = yvalue_final, filt_var = "ever_hit", rep_var = 130, overwrite  = FALSE) %>%
  
nudge_node( . , .filter_col = node_group, .corr_col = yvalue_final, filt_var = "never_hit", rep_var = -120, overwrite  = FALSE) %>%
nudge_node( . , .filter_col = node_group, .corr_col = xvalue_final, filt_var = "never_hit", rep_var = 7, overwrite  = FALSE) %>%
  
nudge_node( . , .filter_col = node_group, .corr_col = yvalue_final, filt_var = "t_sens", rep_var = -20, overwrite  = FALSE) %>%
nudge_node( . , .filter_col = node_group, .corr_col = xvalue_final, filt_var = "t_sens", rep_var = 7.5, overwrite  = FALSE) %>%
  

nudge_node( . , .filter_col = node_group, .corr_col = yvalue_final, filt_var = "tech1", rep_var = 270, overwrite  = FALSE) %>%
nudge_node( . , .filter_col = node_group, .corr_col = xvalue_final, filt_var = "tech1", rep_var = 6, overwrite  = FALSE) %>%
  
nudge_node( . , .filter_col = node_group, .corr_col = yvalue_final, filt_var = "tech2", rep_var = 180, overwrite  = FALSE) %>%
nudge_node( . , .filter_col = node_group, .corr_col = xvalue_final, filt_var = "tech2", rep_var = 6, overwrite  = FALSE) %>%
  
 # nudge_node( . , .filter_col = node_group, .corr_col = xvalue_final, filt_var = "tech1", rep_var = , overwrite  = FALSE) %>%
