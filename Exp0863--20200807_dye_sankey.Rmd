...
y_correction <- tibble("node_group" = df_long$node_group %>% unique) %>%
                    mutate("corr" = lapply(node_group, set_corr) %>% as_vector() ) 
...
df_long_out <- df_long %>%
            left_join(y_correction, by = "node_group") %>%
...
ggsankey(nodes, xvalue_final, yvalue_final) + theme_void()


ggsankey(df_long_out) + theme_void()
```
