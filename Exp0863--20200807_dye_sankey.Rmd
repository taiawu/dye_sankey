...
set_corr <- function( val ) {
  ...
  } else if (val == "trash") {corr <- -200
  } else if (val == "tech1") {corr <- 200
  } else {
    corr <- 100
  }
}
...
# y_correction <- df_long_raw %>% 
#                nest() %>%
#               arrange(check_point, xvalue) %>%
#   select(-data) %>%
#  arrange(check_point, xvalue)
# y_correction

y_correction <- tibble("check_point" = df_long_raw$check_point %>% unique) %>%
                    mutate("corr" = lapply(check_point, set_corr) %>% as_vector() ) #c(-100, 100, 100, 100, 100, 100, 100, 100, 100, 100)) 
...
df_long %>%
  ...
  geom_line(data = df_long, aes(x = xvalue, y = yvalue_corr, group = dye), color = "lightgrey") +
   geom_line(data = df_long %>% filter(reject_buffer == 1), aes(x = xvalue, y = yvalue_corr, group = dye), color = "darkgrey") +
  geom_line() +
  # geom_point() +
